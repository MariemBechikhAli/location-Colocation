...
<html>
    ...
    <body>
      ...
      if (mp.equals(mp1)) 
            {
            String req="insert into locataire values(null,'"+nom+"','"+prenom+"','"+date_naiss+"','"+sexe+"','"+activite+"','"+photo+"','"+adr+"','"+ tel+"','"+mail+"','"+mp+"','"+etat+"','"+qualite+"')"; 
         System.out.print(req);
            C.Ajout(req);
        response.sendRedirect("connexionloc?e-mail="+mail+"&password="+mp);
            }
          else
            {  response.sendRedirect("s'inscrire_err_mdploc.jsp");}
           
             }catch(Exception e){
 
               response.sendRedirect("s'inscrire_erreurloc.jsp");
          }
